...
do imc = istart, iend  ! Loop over initial conditions
  ...
  do j = 0, Ntime
    ...
    norm= OverlapIn*(-1)**Maslov*cdsqrt(pref)&
        * id*cdexp(Iu*Sfwd(j))*(2.d0)**NCl&  !2 *NCl from rho wig
        / (2.d0*pi)**(2*NSC)/(2.d0*pi)**(NCl)/normC
...
if (myrank.eq.0) then
 ...
 normT = normT/dble(NumberMCsteps-badtotE-badtotS)
 ...
 do j = 0, Ntime
   ...
   write(555,'(E15.6)',advance='no'), dimag(TCFT(Ndof,j))
   write(555,'(E15.6)',advance='no'), real(normT(j))
   write(555,'(E15.6)'), dimag(normT(j))

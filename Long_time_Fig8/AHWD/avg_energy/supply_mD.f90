...
!######################################################!
!            Matrix element of B = E                   !
!           Specific for HHB potential                 !
!######################################################!
subroutine DF_B(z,Et)
use parameters, only : Iu, Width0, InverseWidth0, Ndof, NSC, Mass, coupling
implicit none
...
xi      = qb(1) + Iu*InverseWidth0(1,1)*dp(1)/2.d0
eta     = pb(1) - Iu*Width0(1,1)*dq(1)/2.d0
wbath   = 0.333d0 
ky      = Mass(2,2)*wbath**2

! For SC dofs, we have <zt'|B|zt> * 1_W
! For classical dofs, we have (B_Cl)_W * <zt'|zt>
Et = 0.5d0*(eta**2 + Width0(1,1)/2.d0)/Mass(1,1) & ! px^2/2mx
   + 0.5d0*pb(2)**2/Mass(2,2) & ! py^2/2my
   + (xi**2 + 0.5d0/Width0(1,1)) - 0.1d0*(xi**3 + 1.5d0*xi/Width0(1,1)) & !x^2 - 0.1*x^3
   + 0.1d0*(xi**4 + 3.d0*xi**2/Width0(1,1) + 7.5d-1/Width0(1,1)**2)     &    ! + 0.1 x^4
   + coupling*xi*qb(2) + 0.5d0*ky*qb(2)**2                ! kxy + 0.5*my*wy^2*y^2

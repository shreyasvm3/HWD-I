...
do imc = istart, iend  ! Loop over initial conditions
  ...
  do j = 0, Ntime
    ...
    ! TCF estimator
    TCF(1) = OverlapIn*(-1)**Maslov*cdsqrt(pref)&
           * posn(1)*cdexp(Iu*Sfwd(j))*(2.d0)**NCl&  !2 *NCl from rho wig
...
if (myrank.eq.0) then
 ...
 TCFT(1,:) = TCFT(1,:)/dble(NumberMCsteps-badtotE-badtotS)

...
complex*16, allocatable :: Normsum(:), id, norm, normT(:)
...
allocate(Normsum(0:Ntime),normT(0:Ntime))
...
do imc = istart, iend  ! Loop over initial conditions
  ...
  do j = 0, Ntime
    ...
    ! CS matrix element of B and B=1
    call DF_B(qpfwd(1,:,j),qpfwd(2,:,j),qppfwd(1,:,j),qppfwd(2,:,j),posn,id)
    ...
    ! TCF estimator
    TCF = OverlapIn*(-1)**Maslov*(-1)**Maslovp*cdsqrt(pref)*cdsqrt(prefp)*&
          posn*cdexp(Iu*(Sfwd(j)-Spfwd(j)))/normC/(2.d0*pi)**(2*Ndof)
    norm= OverlapIn*(-1)**Maslov*(-1)**Maslovp*cdsqrt(pref)*cdsqrt(prefp)*&
          id*cdexp(Iu*(Sfwd(j)-Spfwd(j)))/normC/(2.d0*pi)**(2*Ndof)

    TCFsum(:,j) = TCFsum(:,j) + TCF
    normsum(j)  = normsum(j)  + norm
...
call mpi_reduce(normsum,normT,Ntime+1,mpi_double_complex,mpi_sum,0,mpi_comm_world,ierr)
call mpi_reduce(bad,badtot,1,mpi_integer,mpi_sum,0,mpi_comm_world,ierr)

if (myrank.eq.0) then

 open(555,file='TCF.out',status='unknown')
 TCFT  = TCFT/dble(NumberMCsteps-badtot)
 normT = normT/dble(NumberMCsteps-badtot)

 do j = 0, Ntime
   TCFT(:,j) = TCFT(:,j)/normT(j) !Normalize
   ...
   write(555,'(E15.6)'), dimag(TCFT(2,j))

   write(666,'(E15.6)',advance='no'), dble(j*TimeStep)
   write(666,'(E15.6)',advance='no'), real(normT(j))
   write(666,'(E15.6)'), dimag(normT(j))

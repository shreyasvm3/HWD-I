...
!                   DF MQC-IVR                         !
!######################################################!
subroutine DF_B(q,p,qp,pp,Bq,id)
use parameters, only : Iu, WidthT, InverseWidthT, Ndof, Mass
implicit none

real*8, intent(in)      :: q(Ndof), p(Ndof), qp(Ndof), pp(Ndof)
complex*16, intent(out) :: Bq(Ndof), id
integer                 :: i
...
do i = 1, Ndof
   Bq(i) = 0.5d0*((q(i)+qp(i))+Iu*InverseWidthT(i,i)*(p(i)-pp(i)))
end do


Bq = Bq*dexp(-0.25d0*dot_product(q-qp,q1)-0.25d0*dot_product(p-pp,p1))&
       *cdexp(-0.5d0*Iu*dot_product(p+pp,q-qp))

! CS overlap for B = 1
id = dexp(-0.25d0*dot_product(q-qp,q1)-0.25d0*dot_product(p-pp,p1))&
   * cdexp(-0.5d0*Iu*dot_product(p+pp,q-qp))

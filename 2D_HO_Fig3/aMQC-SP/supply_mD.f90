...
!                   DF MQC-IVR                         !
!######################################################!
subroutine DF_Bq(lab,q,p,qp,pp,Bq)
...
Bq = 0.5d0*((q(lab)+qp(lab))+Iu*InverseWidthT(lab,lab)*(p(lab)-pp(lab)))&
...
!        only for system block for aMQC-SP             !
!######################################################!
...
real*8                  :: D, G, H, J
real*8                  :: K
complex*16              :: pref1(1,1), pref1p(1,1)

!A  =  matmul(dsqrt(Width0),Mfwd(:,:,1))
!B  =  matmul(A,dsqrt(InverseWidthT))

A  =  dsqrt(Width0(1,1))*Mfwd(1,1,1)
B  =  A*dsqrt(InverseWidthT(1,1))

!C  =  matmul(dsqrt(InverseWidth0),Mfwd(:,:,4))
!D  =  matmul(C,dsqrt(WidthT))
C  =  dsqrt(InverseWidth0(1,1))*Mfwd(1,1,4)
D  =  C*dsqrt(WidthT(1,1))

!G  =  matmul(dsqrt(Width0),Mfwd(:,:,2))
!H  =  matmul(G,dsqrt(WidthT))
G  =  dsqrt(Width0(1,1))*Mfwd(1,1,2)
H  =  G*dsqrt(WidthT(1,1))

!J  =  matmul(dsqrt(InverseWidth0),Mfwd(:,:,3))
!K  =  matmul(J,dsqrt(InverseWidthT))
J  =  dsqrt(InverseWidth0(1,1))*Mfwd(1,1,3)
K  =  J*dsqrt(InverseWidthT(1,1))
...
call zgetrf(1,1,pref1,1,ipiv,info)
...
call zgetrf(1,1,pref1p,1,ipiv2,info)
